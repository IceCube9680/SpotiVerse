...
class DownloadHandler:
    ...
    # ... (other methods remain the same until download_album) ...
    ...
    async def download_album(self, provider, album_id_or_url, user_id, message):
        ...
        try:
            ...
            if provider == "spotify" or provider == 'sp':
                # Extract Spotify ID from URL or string
                spotify_id = self.extract_spotify_id(album_id_or_url)
                if not spotify_id:
                    await self.safe_edit_message(message, "❌ Invalid Spotify URL or ID.")
                    return False
                
                # Determine content type
                content_type = self.determine_spotify_content_type(album_id_or_url)
                
                # Fetch track ids using spotipy
                def fetch_spotify_ids():
                    try:
                        if content_type == "album":
                            results = self.search_handler.spotify.album_tracks(spotify_id)
                        elif content_type == "playlist":
                            results = self.search_handler.spotify.playlist_items(spotify_id)
                        elif content_type == "artist":
                            results = self.search_handler.spotify.artist_top_tracks(spotify_id)
                        else:
                            # Default to track if single track
                            return [spotify_id]
                        
                        track_ids = []
                        if results and 'items' in results:
                            for item in results['items']:
                                track = item.get('track') if content_type == "playlist" else item
                                if track and track.get('id'):
                                    track_ids.append(track['id'])
                        
                        return track_ids[:50]  # Limit to 50 tracks max
                    except Exception as e:
            ...
            progress = await self.safe_edit_message(message, f"⬇️ Preparing to download {total} tracks from album/playlist...\nProgress: 0/{total}")

            for idx, (prov, tid) in enumerate(track_items, start=1):
                if progress:
                    await self.safe_edit_message(progress, f"⬇️ Downloading {idx}/{total}...\nSuccessful: {success_count}  Failed: {fail_count}")
                try:
                    # Create a new progress message for each track
                    track_progress = await self.bot.send_message(
                        chat_id=user_id,
                        text=f"Processing track {idx}/{total}..."
                    )
                    
                    single_success = await self.download_track(prov, tid, user_id, track_progress)
                    if single_success:
                        success_count += 1
                    else:
                        fail_count += 1
                        
                    # Delete track progress message
                    try:
                        await track_progress.delete()
                    except:
                        pass
            ...
            await self.safe_edit_message(progress if progress else message, 
                f"✅ Album/playlist download finished.\nSuccessful: {success_count}\nFailed: {fail_count}")
            return success_count > 0

        except Exception as e:
            logger.error(f"Unexpected error in download_album: {e}\n{traceback.format_exc()}")
            await self.safe_edit_message(message, f"❌ Unexpected error: {str(e)}")
            return False
